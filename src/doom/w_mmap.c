...
void W_DoneCache(void)
{
  if (cachelump) {
    int i;
    for (i = 0; i < numlumps; i++) {
      if (cachelump[i].mmapadr) {
        I_Munmap(cachelump[i].mmapadr, W_LumpLength(i));
        cachelump[i].mmapadr = NULL;
      }
    }
    free(cachelump);
    cachelump = NULL;
...
void W_UnlockLumpNum(int lump) {
  if (cachelump[lump].locks == -1) {
    // Keep the memory mapped lump cached in PSRAM instead of freeing it.
    // This avoids repeated SD card SPI reads on every frame.
    return;
  }
#ifdef SIMPLECHECKS
  if (cachelump[lump].locks == 0)
