...
extern void doom_trigger_gsod(const char* title, const char* message);
...
void I_Error(const char *error, ...)
{
  ...
  // Trigger system Green Screen of Death (GSOD)
  doom_trigger_gsod("Doom Motor Hatasi", errmsg);

  // Block indefinitely so main task can draw GSOD and system doesn't reset instantly
  while (1) {
    vTaskDelay(100);
  }
}
