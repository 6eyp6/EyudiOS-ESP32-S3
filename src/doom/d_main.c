...
extern volatile boolean doomFrameReady;
...
void D_Display (void)
{
  ...
  // V-Sync Handshake: If the screen is busy displaying the previous frame,
  // sleep/yield Core 0. This stops Core 0 from wasting CPU cycles rendering frames
  // that will be discarded, and gives CPU time to background Tasks (feeding watchdog).
  while (doomFrameReady) {
    I_uSleep(1000);
  }
...
static void D_DoomLoop(void)
{
  ...
  M_DoScreenShot(auto_shot_fname);
      ...
      // ESP32 Watchdog feed / Task yield
      I_uSleep(1000);
    }
}
...
//    I_CalculateRes(w, h);
  ...
  D_InitNetGame();
  doomLoadingProgress = 10;
  strcpy(doomLoadingText, "Network init done");

  //jff 9/3/98 use logical output routine
  lprintf(LO_INFO,"W_Init: Init WADfiles.\n");
  W_Init(); // CPhipps - handling of wadfiles init changed
  doomLoadingProgress = 30;
  strcpy(doomLoadingText, "WAD files loaded");
  ...
  R_Init();
  doomLoadingProgress = 50;
  strcpy(doomLoadingText, "Renderer initialized");

  //jff 9/3/98 use logical output routine
  lprintf(LO_INFO,"\nP_Init: Init Playloop state.\n");
  P_Init();
  doomLoadingProgress = 60;
  strcpy(doomLoadingText, "Playloop initialized");

  //jff 9/3/98 use logical output routine
  lprintf(LO_INFO,"I_Init: Setting up machine state.\n");
  I_Init();
  doomLoadingProgress = 70;
  strcpy(doomLoadingText, "Machine state ready");

  //jff 9/3/98 use logical output routine
  lprintf(LO_INFO,"S_Init: Setting up sound.\n");
  S_Init(snd_SfxVolume /* *8 */, snd_MusicVolume /* *8*/ );
  doomLoadingProgress = 80;
  strcpy(doomLoadingText, "Sound initialized");

  //jff 9/3/98 use logical output routine
  lprintf(LO_INFO,"HU_Init: Setting up heads up display.\n");
  HU_Init();
  doomLoadingProgress = 85;
  strcpy(doomLoadingText, "HUD initialized");

  if (!(M_CheckParm("-nodraw") && M_CheckParm("-nosound")))
    I_InitGraphics();
  doomLoadingProgress = 95;
  strcpy(doomLoadingText, "Graphics initialized");

  //jff 9/3/98 use logical output routine
  lprintf(LO_INFO,"ST_Init: Init status bar.\n");
  ST_Init();
  doomLoadingProgress = 100;
  strcpy(doomLoadingText, "Ready!");
