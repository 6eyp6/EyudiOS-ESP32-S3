...
**EyudiOS ESP32-S3 Edition v2.0** is a high-performance open-source operating system kernel and runtime environment created by **Eyüp SAĞLAM (Eyudio)**. It features dual-core FreeRTOS task scheduling, direct 400x300 VGA display output, USB Keyboard/Mouse drivers, the `EsDOS` command shell, the `EyuScript Turbo v3` scripting engine, and a clean modular application registry (`AppRegistry`).
...
* **Input:** CH375 USB Host Module (USB Keyboard & Mouse)
...
- 📖 [docs/architecture.md](docs/architecture.md) ([TR](docs/tr/architecture.md)) — Core Architecture, Concurrency & Task Map.
